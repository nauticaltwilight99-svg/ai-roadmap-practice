...
#   AUTO_APPROVE_SAFE=1         # Auto-run safe tasks; block destructive operations
# ============================================================
...
GEMINI_PYTHON_BIN="${GEMINI_PYTHON_BIN:-/mnt/c/Users/Liza/Desktop/AIProjects/ai-roadmap-practice/venv/Scripts/python.exe}"
GEMINI_ENV_FILE="${GEMINI_ENV_FILE:-/mnt/c/Users/Liza/Desktop/AIProjects/ai-roadmap-practice/.env}"
GEMINI_AGENT_WIN="${GEMINI_AGENT_WIN:-/mnt/c/Users/Liza/Desktop/AIProjects/Auto-Company-main/scripts/core/gemini-agent.py}"
GEMINI_PROJECT_DIR="${GEMINI_PROJECT_DIR:-/mnt/c/Users/Liza/Desktop/AIProjects/Auto-Company-main}"
...
AUTO_APPROVE_SAFE="${AUTO_APPROVE_SAFE:-0}"
RESOLVED_ENGINE_BIN=""

if [ "$ENGINE" != "claude" ] && [ "$ENGINE" != "codex" ] && [ "$ENGINE" != "gemini" ]; then
    echo "Error: ENGINE must be 'claude', 'codex', or 'gemini' (received: '$ENGINE')."
    exit 1
fi
...
resolve_engine_bin() {
    case "$ENGINE" in
        ...
        codex)
            ...
            ;;
        gemini)
            echo "$SCRIPT_DIR/gemini-agent.py"
            ;;
        *)
            return 1
            ;;
...
run_gemini_cycle() {
    local prompt="$1"
    local output_file timeout_flag
    local gemini_python_bin="$GEMINI_PYTHON_BIN"
    local gemini_env_file="$GEMINI_ENV_FILE"
    local gemini_agent_win="$GEMINI_AGENT_WIN"
    local gemini_project_dir="$GEMINI_PROJECT_DIR"

    if [[ "$gemini_python_bin" =~ ^[A-Za-z]:[\\/].* ]]; then
        gemini_python_bin="$(win_path_to_wsl "$gemini_python_bin")"
    fi
    if [[ "$gemini_env_file" =~ ^[A-Za-z]:[\\/].* ]]; then
        gemini_env_file="$(win_path_to_wsl "$gemini_env_file")"
    fi
    if [[ "$gemini_agent_win" =~ ^[A-Za-z]:[\\/].* ]]; then
        gemini_agent_win="$(win_path_to_wsl "$gemini_agent_win")"
    fi
    if [[ "$gemini_project_dir" =~ ^[A-Za-z]:[\\/].* ]]; then
        gemini_project_dir="$(win_path_to_wsl "$gemini_project_dir")"
    fi

    # The configured Gemini interpreter is a Windows executable mounted in WSL.
    # Windows Python must receive Windows paths, not /mnt/* paths.
    if [[ "$gemini_python_bin" =~ \.exe$ ]]; then
        gemini_env_file="$(wsl_path_to_win "$gemini_env_file")"
        gemini_agent_win="$(wsl_path_to_win "$gemini_agent_win")"
        gemini_project_dir="$(wsl_path_to_win "$gemini_project_dir")"
    fi
    ...
    (
        cd "$PROJECT_DIR" || exit 1
        printf '%s' "$prompt" | AUTO_APPROVE_SAFE="$AUTO_APPROVE_SAFE" GEMINI_ENV_FILE="$gemini_env_file" GEMINI_PROJECT_DIR="$gemini_project_dir" "$gemini_python_bin" "$gemini_agent_win"
    ) > "$output_file" 2>&1 &
    local gemini_pid=$!
...
log "Project: $PROJECT_DIR"
    ...
    log "Engine: codex | Model: $MODEL_LABEL | Sandbox: $CODEX_SANDBOX_MODE"
        ...
        log "Engine: gemini | Model: $MODEL_LABEL | API adapter: $RESOLVED_ENGINE_BIN"
else
...
if [ -n "$engine_version" ]; then
    if [ "$ENGINE" = "codex" ]; then
        ...
        log "Gemini adapter: $engine_version"
    else
